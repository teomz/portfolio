        ...
        <ul className='flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5'>
          {
            links.map(link => (
              ...
              >
                <Link className={clsx('flex items-center justify-center px-3 py-3 hover:text-gray-950 transistion',{"text-gray-950":activeSection === link.name})} href={link.hash}
                  onClick={() => {
                ...
                >
                  {link.name}
                  {link.name === activeSection && (<motion.span className='bg-gray-100 rounded-full absolute inset-0 -z-10' layoutId='activeSession' transition={{type:'spring', stiffness:380, damping: 30}}></motion.span>)}
                </Link>
              </motion.li>
            ))
          }
